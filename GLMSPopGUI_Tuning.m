...
% This function creates a GUI for browsing model fits to neural
% responses. The code imports a .mat file called GLMSPopData, which stores,
% among other things, the parameters to these model fits. Each set of
% neural data is fit with two models, and the more appropriate model is
% determined using a normalized log-likelihood difference threshold. A
% particular parameter of each model fit (the preferred direction) is shown
% in the polar histograms (1D model preferred directions in orange and 2D
% preferred directions in blue)
% 
% Functionality:
% Users can browse the model fits to each dataset by selecting it from the 
% list of datafiles in the table. 
%
% The normalized log-likelihood difference threshold can be set by entering 
% a value in the text box, or by clicking in the histogram of normalized 
% log-likelihood differences.
%
% Any individual dataset may be realayzed by selecting it from the table
% and hitting the 'Reanalyze' button.
%
% All datasets can be re-fit by the two models by hitting the 'Reanalyze 
% All' button.
%
% 

% These functions are fun once when the initializing the figure
SetUpFig()
UnpackPopulationData()
resetAll()

% This portion of the code allows for calls to specific subfunctions inside
% of GLMSPoPGUI_Tuning by passing them in as additional parameters.
if ~isempty(varargin)
...
function ReanalAll(~,~,~)
global GLMSPopData
% This function reanalyzes all datafiles in the table, fitting both models
% to each datafile and storing details of the fit back into GLMSPopData.

% Load figure variables
TunFig = get(150,'userdata');
conpanel = get(TunFig.conpanel,'userdata');

% Iterate through each datafile
...
if ismac
    library = '/Users/jpatrickweller/Dropbox/Greg and Patrick/';
elseif ispc
    library = 'C:\Users\jpweller\Dropbox\Greg and Patrick\';
end
...
%conpanel.diffnormLL = conpanel.twoD.bichrom.normLLs - conpanel.oneD.bichrom.normLLs;
conpanel.diffnormLL = conpanel.twoD.bichrom.normLLs - conpanel.twoD.unichrom.normLLs;
